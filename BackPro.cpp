...
void CBackPro::SetParam( UINT uiNumHidden,double dEta, double dTargetError,UINT uiMaxEpoch)
{
	...
	m_vErrorWeightInput2Hidden.resize(m_uiNumInput);
	...
	{				
		...
		m_vErrorWeightInput2Hidden[i].resize(m_uiNumHidden);				
	}	
////////////////////////////////////////////////
	...
	m_vErrorWeightHidden2Output.resize(m_uiNumHidden);	
	...
	{		
		...
		m_vErrorWeightHidden2Output[i].resize(m_uiNumOutput);
	}
...
void CBackPro::AddData(INPUTPATTERN _input)
{	
...
{	
	...
	{		
		...
		sText.Format("%sTotal data : %d kata, %d seri window\r\n",sBuff, m_iJumKata,m_vLearningData.size());
		...
		if (sample == m_iJumKata){	
			...
			if((m_uiIterations >m_uiMaxEpoch) || (m_dMSE<m_dTargetError)){
				if(pStatus!=NULL)
				{
					CString sBuff,sText;
					pStatus->GetWindowText(sBuff);
					sText.Format("%s%d Convergen Best MSE : %3.4f AvgErr : %3.4f\r\n",sBuff,m_uiIterations, m_dMinMSE,m_dMinErrAbs);
					pStatus->SetWindowText(sText);				
				}	
				break;	
			}			
...
void CBackPro::Save(CProgressCtrl * pProg)
{
	...
	m_pDb->Execute("DELETE from TPembelajaran");
...
void CBackPro::FeedForward(INPUTPATTERN inputx)
{
	int i,j;
	/*
	 Feed forward
...
void CBackPro::CalculateError(INPUTPATTERN inputx)
{
	...
	{		
		...
		for(i=0;i<m_uiNumInput;i++)	
		{															
			m_vErrorWeightInput2Hidden[i][j] = err*inputx.input[i];						
		}		
	}	
}
...
void CBackPro::ResetDeltaWeight()
{
	...
	{																			
		...
		for(j=0;j<m_uiNumHidden;j++)
		{
			m_vErrorWeightHidden2Output[j][i] = 0;
		}	
	}
	//hidden layer
	for (i=0;i<m_uiNumHidden;i++)
	{
		...
		for(j=0;j<m_uiNumInput;j++)
		{
			m_vErrorWeightInput2Hidden[j][i] = 0;
		}	
	}
}
...
void CBackPro::UpdateWeight()
{
	...
	for(j=0;j<m_uiNumHidden;j++)
	{
		for(i=0;i<m_uiNumInput;i++)	
		{			
			//m_vWeightInput2Hidden[i][j] += m_dEta*m_vErrorWeightInput2Hidden[i][j];																			
			divisor = m_v2WeightInput2Hidden[i][j] + 0.1;
			if(divisor<0.1)
				divisor = 1.0;
			epsilon = m_dEta / divisor;
			m_vWeightInput2Hidden[i][j] +=  epsilon*m_vErrorWeightInput2Hidden[i][j];																						
		}	
	}
	...
	for(i=0;i<m_uiNumOutput;i++)
	{
		for(j=0;j<m_uiNumHidden;j++)
		{			
			//m_vWeightHidden2Output[j][i] += m_dEta*m_vErrorWeightHidden2Output[j][i];						
			divisor = m_v2WeightHidden2Output[j][i] + 0.1;
			if(divisor<0.1)
				divisor = 1.0;
			epsilon = m_dEta / divisor;
			m_vWeightHidden2Output[j][i] +=  epsilon*m_vErrorWeightHidden2Output[j][i];																						
		}	
	}
}
...
void CBackPro::TotalError(CEdit * pStatus )
{
	...
	if(pStatus!=NULL)
	{
		CString sBuff,sText;
		pStatus->GetWindowText(sBuff);		
		sText.Format("%s%d MSE : %3.4f AvgErr : %3.4f\r\n",sBuff,m_uiIterations+1, m_dMSE, m_dErrAbs);
		//WriteLog(sText);
		pStatus->SetWindowText(sText);						
	}	
}
